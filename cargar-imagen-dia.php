...
if (!file_exists(''.$titulo.'/')) {
    mkdir('img/imagen_dia/'.$titulo.'/', 0777, true);
}

$direccion="img/imagen_dia/$titulo/";
$titulofinal= trim ($_FILES['archivo']['titulo']);
$subido= $direccion . $titulofinal;
$archivo=($_FILES['archivo']['titulo']); // aca traigo el archivo a subir
...
if ((($_FILES["archivo"]["type"] == "image/jpg")
   ...
   && in_array($extensionimg, $extensionespermitidas)) {
    ...
    $config_data = config_read("registro-usuarios.ini");
    config_set($config_data, "$titulo", "titulo", $titulo);
    config_set($config_data, "$titulo", "mensaje", $mensaje);
    config_set($config_data, "$titulo", "imagen", $archivo);
...
echo "<br>";
echo "<a href='imagen_dia.php'>Ver posts publicados";
?>